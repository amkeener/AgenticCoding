...
# Ensure required bin directories are in PATH (for gh, cloudflared, claude, etc.)
extra_paths = [
    "/opt/homebrew/bin",
    "/usr/local/bin",
    os.path.expanduser("~/.local/bin"),  # For Claude Code CLI
]
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        if issue_number:
            ...
            issue_body_preview = issue_body[:200]
            print(f"Issue body preview: {issue_body_preview}")
            print(f"Contains 'adw_': {'adw_' in issue_body.lower()}")
        ...
        if event_type == "issues" and action == "opened" and issue_number:
            ...
            if "adw_" in issue_body.lower():
                print(f"✅ Found 'adw_' in issue body, extracting workflow info...")
                # Use temporary ID for classification
                temp_id = make_adw_id()
                print(f"Calling extract_adw_info with temp_id: {temp_id}")
                workflow, provided_adw_id = extract_adw_info(issue_body, temp_id)
                print(f"extract_adw_info returned: workflow={workflow}, adw_id={provided_adw_id}")
                
                # Fallback: try to extract workflow directly if classifier fails
                if not workflow:
                    print("⚠️  Classifier failed, trying direct extraction...")
                    # Look for patterns like "adw_plan", "adw_build", etc.
                    adw_match = re.search(r'\b(adw_(?:plan|build|test|plan_build|plan_build_test))\b', issue_body.lower())
                    if adw_match:
                        workflow = adw_match.group(1)
                        print(f"✅ Direct extraction found workflow: {workflow}")
                
                if workflow:
                    trigger_reason = f"New issue with {workflow} workflow"
                    print(f"✅ Workflow detected: {workflow}")
                else:
                    print("❌ No workflow detected after all attempts")
        ...
        # Check if this is an issue comment
            ...
            # Check if comment contains "adw_"
                ...
                print(f"✅ Found 'adw_' in comment body, extracting workflow info...")
                # Use temporary ID for classification
                temp_id = make_adw_id()
                print(f"Calling extract_adw_info with temp_id: {temp_id}")
                ...
                if not workflow:
                    print("⚠️  Classifier failed, trying direct extraction...")
                    # Look for patterns like "adw_plan", "adw_build", etc.
                    adw_match = re.search(r'\b(adw_(?:plan|build|test|plan_build|plan_build_test))\b', comment_body.lower())
                    if adw_match:
                        workflow = adw_match.group(1)
                        print(f"✅ Direct extraction found workflow: {workflow}")
        ...
        if workflow:
            print(f"🚀 WORKFLOW TRIGGERED: {workflow}")
            ...
            if not os.path.exists(trigger_script):
                ...
                return {
                    "status": "error",
                    "message": f"Workflow script not found: {workflow}.py"
                }
            ...
            print(f"📄 Script path: {trigger_script}")
            ...
            try:
                ...
                logger.info(f"Launched {workflow} process with PID {process.pid}")
            except Exception as e:
                error_msg = f"❌ Failed to launch workflow: {e}"
                print(error_msg)
                import traceback
                traceback.print_exc()
                logger.error(error_msg)
                return {
                    "status": "error",
                    "message": f"Failed to launch workflow: {str(e)}"
                }
            ...
            if event_type == "issues" and action == "opened":
                print(f"   Issue body check: 'adw_' in content = {'adw_' in (issue.get('body', '') or '').lower()}")
            elif event_type == "issue_comment" and action == "created":
                comment_body = payload.get("comment", {}).get("body", "")
                print(f"   Comment body check: 'adw_' in content = {'adw_' in (comment_body or '').lower()}")
            return {
                "status": "ignored",
