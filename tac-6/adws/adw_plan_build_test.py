...
"""
ADW Plan, Build & Test - AI Developer Workflow with testing (Local-Only Mode)

Usage:
  uv run adw_plan_build_test.py "<description>" [adw-id]

Example:
  uv run adw_plan_build_test.py "Add dark mode toggle to settings page"
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print('Usage: uv run adw_plan_build_test.py "<description>" [adw-id]')
        print("")
        print("Example:")
        print('  uv run adw_plan_build_test.py "Add dark mode toggle to settings page"')
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"Plan-Build-Test workflow finished successfully")
    print(f"ADW ID: {adw_id}")
    print(f"{'='*60}")
