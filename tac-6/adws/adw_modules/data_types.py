...
class LocalIssue(BaseModel):
    """Local issue model for CLI-based workflows without GitHub.

    This model provides compatibility with GitHubIssue for local-only operation.
    Use LocalIssue.from_description() to create from a CLI description string.
    """

    title: str
    body: str
    number: int = 0  # Placeholder for compatibility with GitHubIssue

    @classmethod
    def from_description(cls, description: str) -> "LocalIssue":
        """Create a LocalIssue from a CLI description string.

        The first line (up to 80 chars) becomes the title.
        The full description becomes the body.

        Args:
            description: The issue description from CLI
        ...
        return cls(title=title, body=description)
...
class ADWStateData(BaseModel):
    ...
    # Local-only mode fields
    description: Optional[str] = None  # Full issue description from CLI
    title: Optional[str] = None  # Short title derived from description
