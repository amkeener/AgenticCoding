...
"""
DEPRECATED: GitHub Operations Module - AI Developer Workflow (ADW)

This module is deprecated for local-only ADW operation.
Use LocalIssue and output.log_message() instead.
...
- get_repo_url() - for branch naming
- extract_repo_path() - for local reference
- ADW_BOT_IDENTIFIER - for format compatibility
"""
...
def _deprecated(func):
    """Decorator to mark functions as deprecated for local-only mode."""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        warnings.warn(
            f"{func.__name__}() is deprecated in local-only mode. "
            f"Use LocalIssue and output.log_message() instead.",
            DeprecationWarning,
            stacklevel=2
        )
