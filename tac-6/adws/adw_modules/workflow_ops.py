...
Updated for local-only mode - accepts both GitHubIssue and LocalIssue.
"""
...
# Type alias for issue types (GitHub or Local)
IssueType = Union[GitHubIssue, LocalIssue]
...
def classify_issue(
    issue: IssueType, adw_id: str, logger: logging.Logger
...
def build_plan(
    issue: IssueType, command: str, adw_id: str, logger: logging.Logger
) -> AgentPromptResponse:
...
def create_pull_request(
    ...
    NOTE: Disabled in local-only mode. Returns (None, "Disabled").
    """
    if LOCAL_ONLY_MODE:
        logger.info("PR creation disabled in local-only mode")
        return None, "PR creation disabled in local-only mode"
...
def create_or_find_branch(
    issue_number: str, issue: IssueType, state: ADWState, logger: logging.Logger
