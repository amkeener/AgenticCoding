"""Agent module for executing prompts - supports both local LLM (Ollama) and Claude API.

By default, uses local LLM via Ollama to avoid API costs.
Set USE_LOCAL_LLM=false or use --api flag to use Claude Code CLI instead.
"""
...
# Set USE_LOCAL_LLM=false to use Claude API, otherwise uses local Ollama
USE_LOCAL_LLM = os.getenv("USE_LOCAL_LLM", "true").lower() in ("true", "1", "yes")

# Get Claude Code CLI path from environment (used when USE_LOCAL_LLM=false)
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    """Execute agent with the given prompt configuration.

    Uses local LLM (Ollama) by default. Set USE_LOCAL_LLM=false for Claude API.
    """
    # Check if we should use local LLM
    if USE_LOCAL_LLM:
        from .local_agent import prompt_local_agent
        return prompt_local_agent(request)
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    """Execute a slash command template with arguments.

    Uses local LLM (Ollama) by default. Set USE_LOCAL_LLM=false for Claude API.
    """
    # Check if we should use local LLM
    if USE_LOCAL_LLM:
        from .local_agent import execute_template as local_execute_template
        return local_execute_template(request)
