...
Provides centralized git operations for local-only ADW operation.
Push and PR operations are disabled in local-only mode.
"""
...
# Import GitHub functions from existing module (some still useful locally)
...
def push_branch(branch_name: str) -> Tuple[bool, Optional[str]]:
    ...
    NOTE: Disabled in local-only mode. Returns success without pushing.
    """
    if LOCAL_ONLY_MODE:
        return True, "Push disabled in local-only mode"
...
def check_pr_exists(branch_name: str) -> Optional[str]:
    ...
    NOTE: Disabled in local-only mode. Always returns None.
    """
    if LOCAL_ONLY_MODE:
        return None
...
def finalize_git_operations(state: 'ADWState', logger: logging.Logger) -> None:
    """Git finalization for local-only mode: commit only, no push/PR.

    In local-only mode, this function:
    1. Verifies the branch exists
    2. Logs the final state
    3. Does NOT push to remote
    4. Does NOT create/update PR
    """
    ...
    if LOCAL_ONLY_MODE:
        # Local-only mode: just log info, no push/PR
        logger.info(f"Local git operations complete on branch: {branch_name}")
        logger.info("Push and PR creation disabled in local-only mode")
        logger.info(f"To push manually: git push -u origin {branch_name}")
        return

    # Original GitHub-integrated behavior (kept for reference, not executed in local mode)
    # Always push
