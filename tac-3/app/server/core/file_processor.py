...
from core.sql_processor import validate_identifier
...
def convert_csv_to_sqlite(csv_content: bytes, table_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not validate_identifier(table_name):
            raise ValueError(f"Invalid table name after sanitization: {table_name}")
        ...
        for col in df.columns:
            if not validate_identifier(col):
                raise ValueError(f"Invalid column name after sanitization: {col}")
        
        # Connect to SQLite database
        conn = sqlite3.connect("db/database.db")
        
        # Write DataFrame to SQLite - pandas handles parameterization internally
        df.to_sql(table_name, conn, if_exists='replace', index=False)
        
        # Get schema information - table_name is validated, safe to use
        cursor = conn.cursor()
