...
# Configure logging to stdout with exact format preservation
logging.basicConfig(
    level=logging.INFO,
    format='%(message)s',
    stream=sys.stdout,
...
from core.sql_processor import execute_sql_safely, get_database_schema, validate_identifier
from core.insights import generate_insights
from core.query_history import (
    init_query_history_table,
    save_query_history,
...
# Initialize query history table on startup
init_query_history_table()
...
async def process_natural_language_query(request: QueryRequest) -> QueryResponse:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[SUCCESS] Query saved to history")
        except Exception as e:
            # Log error but don't fail the request
            logger.error(f"[ERROR] Failed to save query to history: {str(e)}")
...
async def delete_table(table_name: str):
    ...
    try:
        ...
        # Drop the table - table_name is validated and confirmed to exist
        # DROP TABLE doesn't support parameterization, but we've validated the identifier
...
async def get_query_history_endpoint() -> QueryHistoryResponse:
    """Get all query history items, ordered by newest first"""
    try:
        queries = get_all_query_history()
        response = QueryHistoryResponse(
            queries=queries,
            total=len(queries)
        )
        logger.info(f"[SUCCESS] Query history retrieved: {len(queries)} queries")
        return response
    except Exception as e:
        logger.error(f"[ERROR] Query history retrieval failed: {str(e)}")
        logger.error(f"[ERROR] Full traceback:\n{traceback.format_exc()}")
...
@app.get("/api/query-history/{query_id}", response_model=QueryHistoryDetailResponse)
async def get_query_history_by_id_endpoint(query_id: int) -> QueryHistoryDetailResponse:
    """Get a specific query history item by ID"""
    try:
        query = get_query_history_by_id(query_id)
        if not query:
            raise HTTPException(status_code=404, detail=f"Query with id {query_id} not found")
        logger.info(f"[SUCCESS] Query history item retrieved: id={query_id}")
        return query
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"[ERROR] Query history item retrieval failed: {str(e)}")
        logger.error(f"[ERROR] Full traceback:\n{traceback.format_exc()}")
        raise HTTPException(500, f"Error retrieving query history: {str(e)}")
