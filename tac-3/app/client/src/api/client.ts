...
export const api = {
  ...
  async getQueryHistory(): Promise<QueryHistoryResponse> {
    return apiRequest<QueryHistoryResponse>('/query-history');
  },
  
  // Get query history by ID
  async getQueryHistoryById(id: number): Promise<QueryHistoryDetailResponse> {
    return apiRequest<QueryHistoryDetailResponse>(`/query-history/${id}`);
  }
};